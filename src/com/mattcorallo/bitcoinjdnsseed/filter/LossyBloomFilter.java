...
package com.mattcorallo.bitcoinjdnsseed.filter;
 ...
 *
 * <p>This is the guava bloom filters, but they randomly leak data.
 *
 *
 ...
 */
@Beta
public final class LossyBloomFilter<T> implements Predicate<T> {
  /**
  ...
  interface Strategy {
     ...
     */
    <T> boolean put(T object, Funnel<? super T> funnel, int numHashFunctions, BitArray bits, Random rnd, int elementCountTarget);
   ...
   */
  private LossyBloomFilter(BitArray bits, int numHashFunctions, Funnel<T> funnel,
      Strategy strategy, int elementCountTarget) {
    ...
    this.elementCountTarget = elementCountTarget;
  }
   ...
   * @since 12.0
   */
  public LossyBloomFilter<T> copy() {
    return new LossyBloomFilter<T>(bits.copy(), numHashFunctions, funnel, strategy, elementCountTarget);
  }
  ...
  private final int elementCountTarget;
  /**
  ...
  public boolean put(T object) {
    return strategy.put(object, funnel, numHashFunctions, bits, rnd, elementCountTarget);
  }
   ...
   * @since 15.0
   */
  public boolean isCompatible(LossyBloomFilter<T> that) {
    checkNotNull(that);
    return (this != that) &&
   ...
   * @since 15.0
   */
  public void putAll(LossyBloomFilter<T> that) {
    checkNotNull(that);
  ...
  public boolean equals(@Nullable Object object) {
    ...
    if (object instanceof LossyBloomFilter) {
      LossyBloomFilter<?> that = (LossyBloomFilter<?>) object;
  ...
  /**
   * Creates a {@link LossyBloomFilter BloomFilter<T>} with the expected number of
   ...
   */
  public static <T> LossyBloomFilter<T> create(
    ...
    try {
      return new LossyBloomFilter<T>(new BitArray(numBits), numHashFunctions, funnel,
          BloomFilterStrategies.MURMUR128_MITZ_32, expectedInsertions);
   ...
   * @return a {@code BloomFilter}
   */
  public static <T> LossyBloomFilter<T> create(Funnel<T> funnel, int expectedInsertions /* n */) {
