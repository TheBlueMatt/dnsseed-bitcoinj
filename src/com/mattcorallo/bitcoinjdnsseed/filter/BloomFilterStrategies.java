 ...
 */
enum BloomFilterStrategies implements LossyBloomFilter.Strategy {
  /**
  ...
  MURMUR128_MITZ_32() {
    ...
    public <T> boolean put(T object, Funnel<? super T> funnel,
                           int numHashFunctions, BitArray bits, Random rnd, int elementCountTarget) {
      ...
      if (bitsChanged) {
        int p = bits.bitSize() / elementCountTarget;
        if (p <= 1 || rnd.nextInt() % p == 0) {
          int nextHash = hash1 + (numHashFunctions + 1) * hash2;
          if (nextHash < 0) {
            nextHash = ~nextHash;
          }
          bits.clear(nextHash % bits.bitSize());
        }
      }
      return bitsChanged;
    }
